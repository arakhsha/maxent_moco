...
exp_dir = f"./output/8a0f_cliffwalk/exp_control_vector0"
...
for i in range(len(alpha_vals)):
    ...
    plot_alg_v(axs[i], f"{exp_dir}/mecvi1_control_{alpha}", rf"MoCoVI ($d$=1)", 7, "solid")
    plot_alg_v(axs[i], f"{exp_dir}/mecvi2_control_{alpha}", rf"MoCoVI ($d$=2)", 4, "solid")
    plot_alg_v(axs[i], f"{exp_dir}/mecvi3_control_{alpha}", rf"MoCoVI ($d$=3)", 5, "solid")
    plot_alg_v(axs[i], f"{exp_dir}/msvi_control_{alpha}", "OS-VI", 2, "dashdot")
    plot_alg_v(axs[i], f"{exp_dir}/vi_control", "VI", 0, "dotted")
    ...
    if i == 0:
        axs[0].set_ylabel(r'Normalized $||V_k - V^*||$')
    if i == 1:
...
plt.savefig(f"{exp_dir}/control_vector.png", bbox_inches="tight")
plt.savefig(f"{exp_dir}/control_vector.pdf", bbox_inches="tight")
...
for i in range(len(alpha_vals)):
    alpha = alpha_vals[i]
    plot_alg_vpi(axs[i], f"{exp_dir}/mecvi1_control_{alpha}", rf"MoCoVI ($d$=1)", 7, "solid")
    plot_alg_vpi(axs[i], f"{exp_dir}/mecvi2_control_{alpha}", rf"MoCoVI ($d$=2)", 4, "solid")
    plot_alg_vpi(axs[i], f"{exp_dir}/mecvi3_control_{alpha}", rf"MoCoVI ($d$=3)", 5, "solid")
    plot_alg_vpi(axs[i], f"{exp_dir}/msvi_control_{alpha}", "OS-VI", 2, "dashdot")

    plot_alg_vpi(axs[i], f"{exp_dir}/vi_control", "VI", 0, "dotted")
    
    Phat = model_class.get_P_hat_using_P(mdp.P(), alpha_vals[i])
    value = get_policy_value(Phat, mdp.R(), mdp.discount(), policy)
    axs[i].axhline(y=value.dot(initial_dist), label=r"Pure MBRL".format(alpha_vals[i]),
                    linestyle="dashed", color=cmap[1])


    if i == 0:
        axs[0].set_ylabel(r'$||V^{\pi_k}||$')
    if i == 1:
        axs[1].set_xlabel("Iteration (k)")
    axs[i].set_xlim(0, 15)
    axs[i].set_xticks([i for i in range(0, 16, 2)])
...
plt.savefig(f"{exp_dir}/control_vector_vpi.png", bbox_inches="tight")
plt.savefig(f"{exp_dir}/control_vector_vpi.pdf", bbox_inches="tight")